...
class Profiler(Casino):
    ...
    def slater_profiling(self):
        ...
        logger.info(' slater tressian v2        %8.1f', (end - start) * 10)
        # stats = rtsys.get_allocation_stats()
        # logger.info(f'{stats} total: {stats[0] - stats[1]}')
    ...
    def jastrow_profiling(self):
        ...
        start = default_timer()
        self.wfn.jastrow.profile_value_parameters_d1(self.dr, self.steps // 10, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' value parameters d1       %8.1f', (end - start) * 10)

        start = default_timer()
        self.wfn.jastrow.profile_laplacian_parameters_d1(self.dr, self.steps // 10, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' laplacian parameters d1   %8.1f', (end - start) * 10)

        start = default_timer()
        self.wfn.jastrow.profile_gradient_parameters_d1(self.dr, self.steps // 10, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' gradient parameters d1    %8.1f', (end - start) *10)
    ...
    def backflow_profiling(self):
        ...
        start = default_timer()
        self.wfn.backflow.profile_value_parameters_d1(self.dr, self.steps // 1000, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' value parameters d1       %8.1f', (end - start) * 1000)

        start = default_timer()
        self.wfn.backflow.profile_laplacian_parameters_d1(self.dr, self.steps // 1000, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' laplacian parameters d1   %8.1f', (end - start) * 1000)

        start = default_timer()
        self.wfn.backflow.profile_gradient_parameters_d1(self.dr, self.steps // 1000, self.atom_positions, self.r_e)
        end = default_timer()
        logger.info(' gradient parameters d1    %8.1f', (end - start) * 1000)
