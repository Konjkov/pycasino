...
@nb.jit(nopython=True, cache=True)
def equilibration(steps, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents):
    """VMC equilibration"""
    i = j = 0
    while i < steps:
        new_X_u = X_u + random_step(dX, neu)
        new_X_d = X_d + random_step(dX, ned)
        new_p = wfn(new_X_u, new_X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents)
        j += 1
        if (new_p/p)**2 > random():
...
@nb.jit(nopython=True, cache=True)
def accumulation(steps, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges):
    ...
    while j < steps:
        ...
        if (new_p/p)**2 > random():
            X_u, X_d, p = new_X_u, new_X_d, new_p
            E[j] = local_energy(X_u, X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges)
            j += 1
    return E


@nb.jit(nopython=True, cache=True)
def averaging_accumulation(steps, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges):
    """VMC accumulation with averaging local energies over proposed moves"""
    E = np.zeros((steps,))
    loc_E = local_energy(X_u, X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges)
    for j in range(steps):
        new_X_u = X_u + random_step(dX, neu)
        new_X_d = X_d + random_step(dX, ned)
        new_p = wfn(new_X_u, new_X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents)
        new_loc_E = local_energy(new_X_u, new_X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges)
...
@nb.jit(nopython=True, cache=True)
def vmc(equlib, stat, mo, neu, ned, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges):
    ...
    p = wfn(X_u, X_d, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents)

    equ = equilibration(equlib, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents)
    print(equlib/equ)

    opt = equilibration(10000, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents)
    print(10000/opt)

    return accumulation(stat, dX, X_u, X_d, p, neu, ned, mo_u, mo_d, shells, primitives, contraction_coefficients, exponents, atomic_positions, atom_charges)
...
if __name__ == '__main__':
    ...
    start = default_timer()
    E = vmc(50000, 1 * 1024 * 1024, gwfn.mo, inp.neu, inp.ned, gwfn.shells, gwfn.primitives, gwfn.contraction_coefficients, gwfn.exponents, gwfn.atomic_positions, gwfn.atom_charges)
    end = default_timer()
